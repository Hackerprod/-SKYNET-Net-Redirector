...
namespace SKYNET.Hook
{
    ...
    /// </summary>
    public class WSASendTo : IHook
    {
        [UnmanagedFunctionPointer(CallingConvention.StdCall, CharSet = CharSet.Unicode, SetLastError = true)]
        private delegate int WSASendToDelegate(IntPtr socketHandle, ref WSABuffer Buffer, int BufferCount, IntPtr bytesTransferred, SocketFlags socketFlags, IntPtr socketAddress, int socketAddressSize, IntPtr overlapped, IntPtr completionRoutine);
        WSASendToDelegate _WSASendTo;

        public override string Library => "ws2_32.dll";
        public override string Method => "WSASendTo";
        ...
        public override Delegate Delegate
        {
            get
            {
                _WSASendTo = Marshal.GetDelegateForFunctionPointer<WSASendToDelegate>(ProcAddress);
                return new WSASendToDelegate(Callback);
            }
        }
        private int Callback(IntPtr socket, ref WSABuffer Buffer, int BufferCount, IntPtr bytesTransferred, SocketFlags socketFlags, IntPtr socketAddress, int socketAddressSize, IntPtr overlapped, IntPtr completionRoutine)
        {
            ...
            try
            {
                ...
                if (address != RedirectedIP || port != RedirectedPort)
                {
                    ...
                    result = _WSASendTo(socket, ref Buffer, BufferCount, bytesTransferred, socketFlags, nAddr, socketAddressSize, overlapped, completionRoutine);
                }
                else
                {
                    result = _WSASendTo(socket, ref Buffer, BufferCount, bytesTransferred, socketFlags, socketAddress, socketAddressSize, overlapped, completionRoutine);
                }
            ...
            {
                return _WSASendTo(socket, ref Buffer, BufferCount, bytesTransferred, socketFlags, socketAddress, socketAddressSize, overlapped, completionRoutine);
            }
